...
type Props = {
  mode: 'login' | 'signup'
  userType?: 'particulier' | 'pro'
  label?: string
}

export default function GoogleSignInButton({ mode, userType, label }: Props) {
  ...
  const handleClick = async () => {
    try {
      ...
      if (mode === 'signup' && userType) {
        document.cookie = `oauth_user_type=${userType}; path=/; max-age=3600; SameSite=Lax`
      }
