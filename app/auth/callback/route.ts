  ...
  try {
    ...
    if (!dbUser) {
      ...
      try {
        ...
        console.error('Error creating user in OAuth callback:', prismaError)
        // If user creation failed due to unique constraint, try to find the user again
        dbUser = await prisma.user.findUnique({
          where: { email: user.email },
        })
        if (!dbUser) {
          redirect('/connexion?error=' + encodeURIComponent('Erreur lors de la création du compte. Veuillez réessayer.'))
        }
      }
    }
